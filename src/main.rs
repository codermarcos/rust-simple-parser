...
fn main() {
	...
	println!("tag with multiple attributes ----------------------");

	println!("{:#?}", parse(String::from("<input id=\"teste\" class=\"campo\" />")));

	// println!("child self close tag with attributes -------------------------------");

	// println!("{:#?}", parse(String::from("<form>Nome: <input id=\"teste\" /> <button>enviar</button></form>")));
